...
describe('SqliteFileStorageProvider (in-memory)', () => {
    ...
    beforeEach(() => {
        provider = new SqliteFileStorageProvider({ dbPath: ':memory:' });
    });
    ...
    describe('remove', () => {
        ...
        it('should remove nested children when removing recursively', async () => {
            await provider.mkdir('/a/b/c', true);
            await provider.writeFile('/a/b/c/file.txt', 'deep');
            await provider.remove('/a', true);
            expect(await provider.exists('/a/b')).toBe(false);
            expect(await provider.exists('/a/b/c')).toBe(false);
            expect(await provider.exists('/a/b/c/file.txt')).toBe(false);
        });
    ...
    describe('move', () => {
        ...
        it('should move a directory with children', async () => {
        ...
        it('should move nested directory structures', async () => {
            await provider.mkdir('/a/b/c', true);
            await provider.writeFile('/a/b/c/deep.txt', 'deep');
            await provider.move('/a', '/x');

            expect(await provider.exists('/a')).toBe(false);
            expect(await provider.exists('/a/b')).toBe(false);
            expect(await provider.readFile('/x/b/c/deep.txt')).toBe('deep');
        });
    });
...
describe('SqliteFileStorageProvider (file-based persistence)', () => {
    ...
    it('should persist data across provider instances', async () => {
        ...
        const provider1 = new SqliteFileStorageProvider({ dbPath: TEST_DB });
        await provider1.mkdir('/docs');
        await provider1.writeFile('/docs/readme.txt', 'Hello from disk!');
        await provider1.writeFile('/root-file.txt', 'root content');

        // Create a new provider pointing at the same file
        const provider2 = new SqliteFileStorageProvider({ dbPath: TEST_DB });

        // Verify all data is available
        const rootEntries = await provider2.list('/');
        expect(rootEntries).toHaveLength(2);
        expect(rootEntries.map(e => e.name)).toEqual(['docs', 'root-file.txt']);

        const content = await provider2.readFile('/docs/readme.txt');
        expect(content).toBe('Hello from disk!');

        const rootContent = await provider2.readFile('/root-file.txt');
        expect(rootContent).toBe('root content');
    });

    it('should persist removes across instances', async () => {
        const provider1 = new SqliteFileStorageProvider({ dbPath: TEST_DB });
        await provider1.writeFile('/temp.txt', 'temporary');
        await provider1.remove('/temp.txt');

        const provider2 = new SqliteFileStorageProvider({ dbPath: TEST_DB });
        expect(await provider2.exists('/temp.txt')).toBe(false);
    });

    it('should persist directory structure across instances', async () => {
        const provider1 = new SqliteFileStorageProvider({ dbPath: TEST_DB });
        await provider1.mkdir('/a/b/c', true);
        await provider1.writeFile('/a/b/c/deep.txt', 'deep content');

        const provider2 = new SqliteFileStorageProvider({ dbPath: TEST_DB });
        const content = await provider2.readFile('/a/b/c/deep.txt');
        expect(content).toBe('deep content');

        const stat = await provider2.stat('/a/b/c');
        expect(stat.type).toBe('directory');
    });

    it('should create parent directories for db file if needed', async () => {
        const nestedPath = `/tmp/test-sqlite-nested-${Date.now()}/sub/data.db`;
        const nestedRoot = `/tmp/test-sqlite-nested-${Date.now()}`;
        try {
            const provider = new SqliteFileStorageProvider({ dbPath: nestedPath });
            await provider.writeFile('/test.txt', 'nested dir test');

            expect(fs.existsSync(nestedPath)).toBe(true);

            const provider2 = new SqliteFileStorageProvider({ dbPath: nestedPath });
            const content = await provider2.readFile('/test.txt');
            expect(content).toBe('nested dir test');
        } finally {
