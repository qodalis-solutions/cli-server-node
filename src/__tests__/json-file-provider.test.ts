...
describe('JsonFileStorageProvider', () => {
    let provider: JsonFileStorageProvider;
    ...
    beforeEach(() => {
        ...
        provider = new JsonFileStorageProvider({ filePath: TEST_FILE });
    });
    ...
    describe('persistence', () => {
        it('should persist data across provider instances', async () => {
            ...
            await provider.mkdir('/docs');
            await provider.writeFile('/docs/readme.txt', 'Hello from disk!');
            await provider.writeFile('/root-file.txt', 'root content');

            // Create a new provider pointing at the same file
            const provider2 = new JsonFileStorageProvider({ filePath: TEST_FILE });
        ...
        it('should persist removes across instances', async () => {
            await provider.writeFile('/temp.txt', 'temporary');
        ...
        it('should persist directory structure across instances', async () => {
            await provider.mkdir('/a/b/c', true);
            await provider.writeFile('/a/b/c/deep.txt', 'deep content');
        ...
        it('should start fresh when JSON file does not exist', async () => {
            const freshPath = `/tmp/test-fs-fresh-${Date.now()}.json`;
            try {
                const freshProvider = new JsonFileStorageProvider({ filePath: freshPath });
                const entries = await freshProvider.list('/');
                expect(entries).toEqual([]);
                expect(await freshProvider.exists('/')).toBe(true);
            } finally {
        ...
        it('should create parent directories for JSON file if needed', async () => {
            const nestedPath = `/tmp/test-fs-nested-${Date.now()}/sub/data.json`;
            const nestedDir = nestedPath.substring(0, nestedPath.lastIndexOf('/'));
            try {
                const nestedProvider = new JsonFileStorageProvider({ filePath: nestedPath });
                await nestedProvider.writeFile('/test.txt', 'nested dir test');

                expect(fs.existsSync(nestedPath)).toBe(true);

                const provider2 = new JsonFileStorageProvider({ filePath: nestedPath });
                const content = await provider2.readFile('/test.txt');
                expect(content).toBe('nested dir test');
            } finally {
                if (fs.existsSync(nestedPath)) {
                    fs.rmSync(nestedPath.substring(0, nestedPath.indexOf('/sub')), { recursive: true });
                }
            }
        });
    });
